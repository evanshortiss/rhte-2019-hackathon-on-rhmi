...
:postgres-hostname: Postgres.city-of-losangeles.svc
:postgres-port: 5432
:postgres-database: city-info
:postgres-connection-url: jdbc:Postgres://{postgres-hostname}:{postgres-port}/{postgres-database}
...
const connectionString = env
  .get(
    ...
    'Postgres://{username}:Password1@localhost:{postgres-port}/{postgres-database}'
  )
  .asUrlString();
----
...
[subs="attributes+"]
----
oc set env dc/nodejs-city-api-server PG_CONNECTION_STRING=Postgres://{username}:Password1@{postgres-hostname}:{postgres-port}/{postgres-database}
----
