...
. Create a GitHub account at link:https://github.com/join[github.com/join] if you don't already have one.
. Sign into your GitHub account.
. Open the link:https://github.com/evanshortiss/rhte-2019-hackathon-on-rhmi-nodejs-api-server-template[Node.js Template] and click the *Fork* button.
...
. Copy a JSON format version of your OpenAPI Spec into the folder and rename it `openapi-spec.json`.
. Run development server by issuing the `npm run start-dev` command.

[type=verification]
The Node.js server should have printed a `server listening on port 8080` message and accessing link:http://localhost:8080[http://localhost:8080] should render API Docs from your OpenAPI Spec JSON file.
...
[NOTE]
====
* This SQL query simply returns all Junctions without their latest status.
* Use this as a template to build out your API endpoints and SQL queries.
====
...
const connectionString = env
  ...
  .asUrlString();
----
.. If you'd like to setup a PostgreSQL database locally to test your changes the data and setup script at this link:https://github.com/evanshortiss/rhte-2019-hackathon-on-rhmi/tree/master/content/db-setup/sync-files[link] will be useful.
...
. Clikc the *add the base URL of your API and save the configuration.* button.
