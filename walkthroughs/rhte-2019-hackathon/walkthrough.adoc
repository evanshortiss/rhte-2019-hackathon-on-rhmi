...
In this section we will create two connections - one to AMQ Streams and one to PostgreSQL.
We will then create an integration that uses these two connections and translates the incoming data from AMQ streams into database *INSERT* statements on PostgreSQL.
...
To allow Fuse Online to write data consumed from AMQ Streams to PostgreSQL, we need to create a new Database connection in Red Hat Fuse Online.
...
The APIs created should combine data from the provided Lookup Tables in PostgreSQL - *{postgres-junction-lookup-table}* and *{postgres-meter-lookup-table}* - with the data from the live junction and meter tables which is being injested from AMQ Streams.

=== Database Table Schemas

The schemas for each of the available database tables are provided below.
...
Should support an optional `status` query parameter to filter the list of meters based on their status.
...
You have been given access to a PostgreSQL database that contains lookup and live data for the traffic
junctions and parking meters. Use the following details to connect from your application running on the cluster:
...
To facilitate rapid development locally, you need to connect to the PostgreSQL instance running on OpenShift.
To do this, you create a port forward from your local workstation to the PostgreSQL pod running in OpenShift.
...
. Get the name of the PostgreSQL pod by running the following command:
+
----
