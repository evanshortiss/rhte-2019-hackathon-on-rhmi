...
provides a React-based implementation of their UI components.
...
For the Parking Meters page you'll:
...
The end result will look similar to the images below:
...
=== Create the UI GitHub Repository & Import to CodeReady Workspaces
...
. In the top menu of the CodeReady Workspace UI click the blue *Run* arrow. Click the *newCustom* item that appears.
. A dialog with numerous input fields will be displayed:
...
. Paste the following content into the _junctions.json_:
+
[subs="attributes+"]
----
[
...
====
. Open the _TrafficMap.tsx_ file.
. Import the Junction Object type using this code:
+
[subs="attributes+"]
----
...
[subs="attributes+"]
----js
// Uses an internal dev server
const DEV_API_URL = 'http://localhost:9000/junctions'
// Replace with your 3scale API url
const LIVE_API_URL = 'YOUR_LIVE_API_URL'
