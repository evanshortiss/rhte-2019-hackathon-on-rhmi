...
=== Creating the integration between AMQ Streams and Postgres
...
[subs="attributes+"]
----
INSERT INTO {postgres-junction-status-table} (junction_id, timestamp, count_ns, count_ew) VALUES (#junction_id, to_timestamp(#timestamp), #count_ns, #count_ew);
----
